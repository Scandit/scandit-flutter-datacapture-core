...
class CoreMethodHandler {
  ...
  Future<void> registerZoomLevelListener() async {
    return await executeCore('CoreModule', 'registerZoomLevelListener', {});
  }

  /// Unregisters the zoom level event listener
  Future<void> unregisterZoomLevelListener() async {
    return await executeCore('CoreModule', 'unregisterZoomLevelListener', {});
  }

  /// Selects the zoom level on the ZoomSwitchControl. Returns the actual zoom level applied.
  Future<num> selectZoomLevel({required int viewId, required double zoomLevel}) async {
    final params = {
      'viewId': viewId,
      'zoomLevel': zoomLevel,
    };
    final result = await executeCore('CoreModule', 'selectZoomLevel', params);
